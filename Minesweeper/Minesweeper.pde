...
void drawTile(int row, int col) {
  ...
  if (place.cleared()) {
    ...
    if (place.flagged()) {
      ...
      rect(row + (SQUARE_SIZE / 2) - (SQUARE_SIZE / 20), col + SQUARE_SIZE / 5, SQUARE_SIZE / 10, 3 * SQUARE_SIZE / 5);
      fill(#CE3636);
      triangle(row + 9 * SQUARE_SIZE / 20, col + SQUARE_SIZE / 5, row + 9 * SQUARE_SIZE / 20, col + SQUARE_SIZE / 2, row + (SQUARE_SIZE - SQUARE_SIZE / 10), col + 17);
    } else {
      fill(#26C627);
      square(row, col, SQUARE_SIZE);
    }
